...
Informally, the \vocab{cardinality} of $S$, denoted $|S|$, is a measure of its ``size''. We will be able to give a nicer definition of cardinality later, once we have discussed bijections, but the following provides a recursive definition of the cardinality for a finite set:

\begin{definition}[Finiteness and the cardinality of a finite set]
The empty set $\emptyset$ is finite, with $|\emptyset|=0$.

$S$ is \vocab{finite} with $|S|=n+1$, if there exists $s \in S$ such that $|S\setminus \{s\}| = n$ for some $n \in \ZZ^+$. We call $|S|$ the \vocab{cardinality} of $S$. 

Any set that is not finite is said to be \vocab{infinite}.
\end{definition}
...
\begin{definition}
The empty set $\emptyset$ is finite and has cardinality $|\emptyset|\coloneqq0$. A non-empty set $X$ is said to be \vocab{finite} and have cardinality $|X|=n\in\ZZ^+$ if and only if there exists a bijection from $X$ to $\ZZ_n$.
\end{definition}
...
\begin{definition}
$X$ is \vocab{countably infinite} if it has the same cardinality as the set $\ZZ^+$.
\end{definition}

\begin{definition}
$X$ is \vocab{countable} if it is either finite or infinitely countable.
\end{definition}
...
\begin{definition}
A function $f$ is \vocab{strictly monotonic increasing} if for all $x_1,x_2\in D_f$,
\[ x_2>x_1\iff f(x_2)>f(x_1). \]
\end{definition}

\begin{definition}
A function $f$ is \vocab{strictly monotonic decreasing} if for all $x_1,x_2\in D_f$,
\[ x_2>x_1\iff f(x_2)<f(x_1). \]
\end{definition}

\begin{definition}
A function $f$ is \vocab{increasing} on an interval $I$ if for all $x_1,x_2\in I$ with $x_1<x_2$, $f(x_1)\le f(x_2)$.
\end{definition}

\begin{definition}
A function $f$ is \vocab{decreasing} on an interval $I$ if for all $x_1,x_2\in I$ with $x_1<x_2$, $f(x_1)\ge f(x_2)$.
\end{definition}
