...
The set of all points $x_n$ is the \vocab{range} of $(x_n)$; the range of a sequence may be a finite set or it may be infinite.

The sequence $(x_n)$ is said to be \vocab{bounded}\index{bounded sequence} if its range is bounded.
\end{definition}
...
Given a sequence $(x_n)$, consider a sequence $(n_k)$ of positive integers such that $n_1<n_2<\cdots$. Then $(x_{n_i})$ is called a \vocab{subsequence}\index{subsequence} of $(x_n)$. If $(x_{n_i})$ converges, its limit is called a \textbf{subsequential limit} of $(x_n)$.
\end{definition}
...
are called the \vocab{upper limit} and \vocab{lower limit} of $(x_n)$ respectively.
\end{definition}
...
\[s_n=\sum_{k=1}^n a_k,\]
which we call a \vocab{series}. The term $s_n$ is called the \textbf{$n$-th partial sum} of the series.
...
is called a \vocab{power series}\index{power series}. The numbers $c_n$ are called the \textbf{coefficients} of the series.
\end{definition}
...
and call $\sum c_n$ the \vocab{product}\index{product of series} of the two given series.
\end{definition}
...
we say that $\sum a_n^\prime$ is a \vocab{rearrangement} of $\sum a_n$.
\end{definition}