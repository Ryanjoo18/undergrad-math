...
\begin{definition}
For any $x\in X$, $r>0$, the \vocab{open ball} centred at $x$ with radius $r$ is defined as
\[B_r(x)\coloneqq\{y\in X\mid d(x,y)<r\}.\]

Similarly, the \vocab{closed ball} centred at $x$ with radius $r$ is defined as
...
A set $N\subset X$ is called a \vocab{neighbourhood} of $x\in X$ if it contains a ball $B_r(x)$ for some $r>0$.
\end{definition}
...
$E$ is \vocab{open} if it is a neighbourhood of each of its elements, i.e., for every $x\in E$ there exists some open ball $B_r(x)\subset E$ for some $r>0$.
\end{definition}
