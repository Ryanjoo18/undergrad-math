...
$V$ is a \vocab{vector space} over $\FF$ if the following properties hold:
...
\end{align*}
With these definitions, $\FF^\infty$ becomes a vector space over $\FF$, as you should verify. The additive identity in this vector space is $\vb{0}=(0,0,\dots)$.
\end{example}
...
For $f,g\in\FF^S$, the sum $f+g\in\FF^S$ is the function defined by
\[(f+g)(x)=f(x)+g(x)\quad(\forall x\in S)\]

For $\lambda\in\FF$, $f\in\FF^S$, the product $\lambda f\in\FF^S$ is the function defined by
\[(\lambda f)(x)=\lambda f(x)\quad(\forall x\in S)\]
\end{notation}

\begin{example}
If $S=[0,1]$ and $\FF=\RR$, then $\RR^{[0,1]}$ is the set of real-valued functions on the interval $[0,1]$.
\end{example}

\begin{example}
If $S$ is a non-empty set, then $\FF^S$ (with the operations of addition and scalar multiplication as defined above) is a vector space over $\FF$.

Additive identity of $\FF^S$ is the function $0:S\to\FF$ defined by
\[0(x)=0\quad(\forall x\in S)\]

For $f\in\FF^S$, additive inverse of $f$ is the function $-f:S\to\FF$ defined by
\[(-f)(x)=-f(x)\quad(\forall x\in S)\]
\end{example}

It is easy to see that $\FF^n$ and $\FF^\infty$ are special cases of the vector space $\FF^S$ because a list of length $n$ of numbers in $\FF$ can be thought of as a function from $\{1,2,\dots,n\}$ to $\FF$ and a sequence of numbers in $\FF$ can be thought of as a function from the set of positive integers to $\FF$. In other words, we can think of $\FF^n$ as $\FF^{\{1,2,\dots,n\}}$ and we can think of $\FF^\infty$ as $\FF^{\{1,2,\dots\}}$.

\section{Subspaces}
\begin{definition}[Subspace]
$U\subset V$ is a \vocab{subspace} of $V$ if $U$ is also a vector space (with the same addition and scalar multiplication as on $V$).
\end{definition}

\begin{lemma}[Conditions for a subspace]\label{lemma:subspace-conditions}
$U\subset V$ is a subspace of $V$ if and only if $U$ satisfies the following conditions:
\begin{enumerate}[label=(\roman*)]
...
\begin{remark}
The three conditions in \cref{lemma:subspace-conditions} usually enable us to determine quickly whether a given subset of $V$ is a subspace of $V$.
\end{remark}
...
\begin{proposition}
Suppose $U_1,\dots,U_n$ are subspaces of $V$. Then $U_1+\cdots+U_n$ is the smallest subspace of $V$ containing $U_1,\dots,U_n$.
\end{proposition}

\begin{proof}
It is easy to see that $\vb{0}\in U_1+\cdots+U_n$ and that $U_1+\cdots+U_n$ is closed under addition and scalar multiplication. Hence $U_1+\cdots+U_n$ is a subspace of $V$.
...
Suppose $U_1,\dots,U_n$ are subspaces of $V$. The sum $U_1+\cdots+U_n$ is called a \vocab{direct sum} if each element of $U_1+\cdots+U_n$ can be written in only one way a sum of $u_1+\cdots+u_n$, $u_i\in U_i$. In this case, we denote the sum as
...
Suppose $V_1,\dots,V_n$ are subspaces of $V$, $W=V_1+\cdots+V_n$. Then the following are equivalent:
\begin{enumerate}[label=(\roman*)]
...
\begin{proposition}
Suppose $U$ and $W$ are subspaces of $V$. Then $U+W$ is a direct sum if and only if $U\cap W=\{\vb{0}\}$.
\end{proposition}

\begin{proof}
First suppose that $U+W$ is a direct sum. If $v\in U\cap W$, then $\vb{0}=v+(-v)$, where $v\in U$, $-v\in W$. By the unique representation of $\vb{0}$ as the sum of a vector in $U$ and a vector in $W$, we have $v=\vb{0}$. Thus $U\cap W=\{\vb{0}\}$.

Now suppose $U\cap W=\{\vb{0}\}$. To prove that $U+W$ is a direct sum, suppose $u\in U$, $w\in W$, and
\[0=u+w.\]
$u=-w\in W$, thus $u\in U\cap W$, so $u=w=\vb{0}$. By \cref{lemma:condition-direct-sum}, $U+W$ is a direct sum.
\end{proof}
\pagebreak
